...
const _sfc_main = {
  ...
  onLoad({
    ...
    if (!local_storage_key) {
      common_vendor.index.__f__("error", "at uni_modules/uni-upgrade-center-app/pages/upgrade-popup.vue:149", "local_storage_key为空，请检查后重试");
      common_vendor.index.navigateBack();
      return;
    }
    const localPackageInfo = common_vendor.index.getStorageSync(local_storage_key);
    if (!localPackageInfo) {
      common_vendor.index.__f__("error", "at uni_modules/uni-upgrade-center-app/pages/upgrade-popup.vue:156", "安装包信息为空，请检查后重试");
      common_vendor.index.navigateBack();
      return;
    }
    ...
    for (let key in localPackageInfo) {
      if (requiredKey.indexOf(key) !== -1 && !localPackageInfo[key]) {
        common_vendor.index.__f__("error", "at uni_modules/uni-upgrade-center-app/pages/upgrade-popup.vue:164", `参数 ${key} 必填，请检查后重试`);
        common_vendor.index.navigateBack();
        return;
      }
    }
